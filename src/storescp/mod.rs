...
fn run(scu_stream: TcpStream, args: &Options, cb: &Function<(Event, String), String>) -> Result<(), Whatever> {
    let Options {
        verbose,
        calling_ae_title,
        strict,
    ...
    loop {
        match association.receive() {
            ...
            Err(err @ dicom_ul::association::server::Error::Receive { .. }) => {
                let _ = cb.call((Event::Error, err.to_string()));
                if verbose {
    ...
    if let Ok(peer_addr) = association.inner_stream().peer_addr() {
        let _ = cb.call((Event::Success, "Success".to_string()));
        info!(
...
impl StoreSCP {
    ...
    #[napi(constructor)]
    pub fn new(port: u16, dir: Option<String>) -> Self {
        StoreSCP {
            port: port,
            dir: dir
        }
    ...
    #[napi]
    pub fn listen(&self, cb: Function<(Event, String), String>) -> Result<(), JsError> {
        ...
        match listener {
            Ok(l) => {
                ...
                let _ = cb.call((Event::Error,"Test".to_string()));
                for stream in l.incoming() {
                    match stream {
                        Ok(scu_stream) => {
                            if let Err(e) = run(scu_stream, &args, &cb) {
